 ...
 *
 * @param <D> the type that this loader generates
 */
...
public abstract class ExecutorServiceLoader<D> extends Loader<ResultOrException<D, Exception>> {
     ...
     *
     * @return a ResultOrException with the results of the load
     */
    ...
    protected void onStartLoading() {
        ...
        if (data != null) {
            if (LOG_DEBUG) {
                Log.v(LOG_TAG, "Delivering existing result=" + data);
            }
            deliverResult(data);
    ...
    private void dispatchOnLoadComplete(final ResultOrException<D, Exception> data) {
        ...
        if (isAbandoned()) {
            if (LOG_DEBUG) {
                Log.d(LOG_TAG, "Loader was abandoned, not delivering");
            }
        } else if (isStarted()) {
            if (LOG_DEBUG) {
                Log.v(LOG_TAG, "Delivering new result=" + data);
            }
            deliverResult(data);
        } else {
            if (LOG_DEBUG) {
                Log.d(LOG_TAG, "Loader was stopped, not delivering");
            }
        }
    }
